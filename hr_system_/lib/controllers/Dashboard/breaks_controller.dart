...
class BreakController extends GetxController {
  ...
  Future<void> fetchBreaks() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse("http://192.168.1.223/api/Breaks"),
  ...
  Future<String?> startBreak(String breakId) async {
    ...
    final res = await http.post(
      Uri.parse('http://192.168.1.223/api/Breaks/start-break'),
      headers: {
  ...
  Future<String?> endBreak(String breakReportId) async {
    ...
    final res = await http.post(
      Uri.parse('http://192.168.1.223/api/Breaks/end-break'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json',
        'accept': '*/*',
  ...
  Future<Duration?> getRemainingTime(String breakId) async {
    ...
    final res = await http.get(
      Uri.parse(
        'http://192.168.1.223/api/Breaks/breaks/$breakId/remaining-time',
      ),
      headers: {'Authorization': 'Bearer $token', 'accept': '*/*'},
    );
    if (res.statusCode == 200) {
